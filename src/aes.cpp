#include "aes.h"

constexpr uint8_t SBOX[16][16] = {
    {0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76},
    {0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0},
    {0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15},
    {0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75},
    {0x09, 0x83, 0x2c, 0x1a, 0x1b, 0x6e, 0x5a, 0xa0, 0x52, 0x3b, 0xd6, 0xb3, 0x29, 0xe3, 0x2f, 0x84},
    {0x53, 0xd1, 0x00, 0xed, 0x20, 0xfc, 0xb1, 0x5b, 0x6a, 0xcb, 0xbe, 0x39, 0x4a, 0x4c, 0x58, 0xcf},
    {0xd0, 0xef, 0xaa, 0xfb, 0x43, 0x4d, 0x33, 0x85, 0x45, 0xf9, 0x02, 0x7f, 0x50, 0x3c, 0x9f, 0xa8},
    {0x51, 0xa3, 0x40, 0x8f, 0x92, 0x9d, 0x38, 0xf5, 0xbc, 0xb6, 0xda, 0x21, 0x10, 0xff, 0xf3, 0xd2},
    {0xcd, 0x0c, 0x13, 0xec, 0x5f, 0x97, 0x44, 0x17, 0xc4, 0xa7, 0x7e, 0x3d, 0x64, 0x5d, 0x19, 0x73},
    {0x60, 0x81, 0x4f, 0xdc, 0x22, 0x2a, 0x90, 0x88, 0x46, 0xee, 0xb8, 0x14, 0xde, 0x5e, 0x0b, 0xdb},
    {0xe0, 0x32, 0x3a, 0x0a, 0x49, 0x06, 0x24, 0x5c, 0xc2, 0xd3, 0xac, 0x62, 0x91, 0x95, 0xe4, 0x79},
    {0xe7, 0xc8, 0x37, 0x6d, 0x8d, 0xd5, 0x4e, 0xa9, 0x6c, 0x56, 0xf4, 0xea, 0x65, 0x7a, 0xae, 0x08},
    {0xba, 0x78, 0x25, 0x2e, 0x1c, 0xa6, 0xb4, 0xc6, 0xe8, 0xdd, 0x74, 0x1f, 0x4b, 0xbd, 0x8b, 0x8a},
    {0x70, 0x3e, 0xb5, 0x66, 0x48, 0x03, 0xf6, 0x0e, 0x61, 0x35, 0x57, 0xb9, 0x86, 0xc1, 0x1d, 0x9e},
    {0xe1, 0xf8, 0x98, 0x11, 0x69, 0xd9, 0x8e, 0x94, 0x9b, 0x1e, 0x87, 0xe9, 0xce, 0x55, 0x28, 0xdf},
    {0x8c, 0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41, 0x99, 0x2d, 0x0f, 0xb0, 0x54, 0xbb, 0x16}};

constexpr uint8_t INV_SBOX[16][16] = {
    {0x52, 0x09, 0x6a, 0xd5, 0x30, 0x36, 0xa5, 0x38, 0xbf, 0x40, 0xa3, 0x9e, 0x81, 0xf3, 0xd7, 0xfb},
    {0x7c, 0xe3, 0x39, 0x82, 0x9b, 0x2f, 0xff, 0x87, 0x34, 0x8e, 0x43, 0x44, 0xc4, 0xde, 0xe9, 0xcb},
    {0x54, 0x7b, 0x94, 0x32, 0xa6, 0xc2, 0x23, 0x3d, 0xee, 0x4c, 0x95, 0x0b, 0x42, 0xfa, 0xc3, 0x4e},
    {0x08, 0x2e, 0xa1, 0x66, 0x28, 0xd9, 0x24, 0xb2, 0x76, 0x5b, 0xa2, 0x49, 0x6d, 0x8b, 0xd1, 0x25},
    {0x72, 0xf8, 0xf6, 0x64, 0x86, 0x68, 0x98, 0x16, 0xd4, 0xa4, 0x5c, 0xcc, 0x5d, 0x65, 0xb6, 0x92},
    {0x6c, 0x70, 0x48, 0x50, 0xfd, 0xed, 0xb9, 0xda, 0x5e, 0x15, 0x46, 0x57, 0xa7, 0x8d, 0x9d, 0x84},
    {0x90, 0xd8, 0xab, 0x00, 0x8c, 0xbc, 0xd3, 0x0a, 0xf7, 0xe4, 0x58, 0x05, 0xb8, 0xb3, 0x45, 0x06},
    {0xd0, 0x2c, 0x1e, 0x8f, 0xca, 0x3f, 0x0f, 0x02, 0xc1, 0xaf, 0xbd, 0x03, 0x01, 0x13, 0x8a, 0x6b},
    {0x3a, 0x91, 0x11, 0x41, 0x4f, 0x67, 0xdc, 0xea, 0x97, 0xf2, 0xcf, 0xce, 0xf0, 0xb4, 0xe6, 0x73},
    {0x96, 0xac, 0x74, 0x22, 0xe7, 0xad, 0x35, 0x85, 0xe2, 0xf9, 0x37, 0xe8, 0x1c, 0x75, 0xdf, 0x6e},
    {0x47, 0xf1, 0x1a, 0x71, 0x1d, 0x29, 0xc5, 0x89, 0x6f, 0xb7, 0x62, 0x0e, 0xaa, 0x18, 0xbe, 0x1b},
    {0xfc, 0x56, 0x3e, 0x4b, 0xc6, 0xd2, 0x79, 0x20, 0x9a, 0xdb, 0xc0, 0xfe, 0x78, 0xcd, 0x5a, 0xf4},
    {0x1f, 0xdd, 0xa8, 0x33, 0x88, 0x07, 0xc7, 0x31, 0xb1, 0x12, 0x10, 0x59, 0x27, 0x80, 0xec, 0x5f},
    {0x60, 0x51, 0x7f, 0xa9, 0x19, 0xb5, 0x4a, 0x0d, 0x2d, 0xe5, 0x7a, 0x9f, 0x93, 0xc9, 0x9c, 0xef},
    {0xa0, 0xe0, 0x3b, 0x4d, 0xae, 0x2a, 0xf5, 0xb0, 0xc8, 0xeb, 0xbb, 0x3c, 0x83, 0x53, 0x99, 0x61},
    {0x17, 0x2b, 0x04, 0x7e, 0xba, 0x77, 0xd6, 0x26, 0xe1, 0x69, 0x14, 0x63, 0x55, 0x21, 0x0c, 0x7d}};

// Galois Multiplication lookup tables
// See https://crypto.stackexchange.com/questions/71204/how-are-these-aes-mixcolumn-multiplication-tables-calculated
static constexpr uint8_t GF_MUL_TABLE[15][256] = {
    {},

    // mul 1
    // reduce branch
    {},

    // mul 2
    {},

    // mul 3
    {},

    {},
    {},
    {},
    {},
    {},

    // mul 9
    {},

    {},

    // mul 11
    {},

    {},

    // mul 13
    {},

    // mul 14
    {}};

/// circulant MDS matrix
static constexpr uint8_t CMDS[4][4] = {{2, 3, 1, 1}, {1, 2, 3, 1}, {1, 1, 2, 3}, {3, 1, 1, 2}};

/// Inverse circulant MDS matrix
static constexpr uint8_t INV_CMDS[4][4] = {{14, 11, 13, 9}, {9, 14, 11, 13}, {13, 9, 14, 11}, {11, 13, 9, 14}};

static constexpr uint8_t RCON[11] = {0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36};

// instantiate the template for static library
template class AES<AESKeyLength::AES_128>;
template class AES<AESKeyLength::AES_192>;
template class AES<AESKeyLength::AES_256>;

template <AESKeyLength KeyLength>
std::unique_ptr<uint8_t[]>
AES<KeyLength>::encrypt_cbc(const uint8_t in[], size_t len, const uint8_t key[], const uint8_t iv[]) {
    check_length(len);
    auto out = std::make_unique<uint8_t[]>(len);
    uint8_t block[block_size];
    uint8_t round_keys[4 * Nb * (Nr + 1)];
    key_expansion(key, round_keys);
    // initialize the first block
    if (iv)
        memcpy(block, iv, block_size);
    else
        memset(block, 0xff, block_size);
    // chipher block by block
    for (size_t i = 0; i < len; i += block_size) {
        xor_blocks(block, in + i, block_size);
        encrypt_block(block, out.get() + i, round_keys);
        memcpy(block, out.get() + i, block_size);
    }
    return out;
}

template <AESKeyLength KeyLength>
std::unique_ptr<uint8_t[]>
AES<KeyLength>::decrypt_cbc(const uint8_t in[], size_t len, const uint8_t key[], const uint8_t iv[]) {
    check_length(len);
    auto out = std::make_unique<uint8_t[]>(len);
    uint8_t block[block_size];
    uint8_t round_keys[4 * Nb * (Nr + 1)];
    key_expansion(key, round_keys);
    // initialize the first block
    if (iv)
        memcpy(block, iv, block_size);
    else
        memset(block, 0xff, block_size);
    // chipher block by block
    for (size_t i = 0; i < len; i += block_size) {
        decrypt_block(in + i, out.get() + i, round_keys);
        xor_blocks(out.get() + i, block, block_size);
        memcpy(block, in + i, block_size);
    }
    return out;
}

template <AESKeyLength KeyLength>
void
AES<KeyLength>::xor_blocks(uint8_t a[], const uint8_t b[], size_t len) {
    for (size_t i = 0; i < len; i++) {
        a[i] ^= b[i];
    }
}

template <AESKeyLength KeyLength>
void
AES<KeyLength>::check_length(size_t len) {
    if (len % block_size != 0) {
        throw std::length_error("plaintext length must be divisible by " + std::to_string(block_size));
    }
}

template <AESKeyLength KeyLength>
void
AES<KeyLength>::encrypt_block(const uint8_t in[], uint8_t out[], const uint8_t round_keys[]) {
    uint8_t state[4 * Nb];

    // rotate the input block
    for (auto i = 0; i < 4; i++) {
        for (auto j = 0; j < Nb; j++) {
            state[i * Nb + j] = in[i + 4 * j];
        }
    }

    add_round_key(state, round_keys);

    for (auto round = 1; round <= Nr - 1; round++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, round_keys + round * 4 * Nb);
    }

    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, round_keys + Nr * 4 * Nb);

    // rotate the block back
    for (auto i = 0; i < 4; i++) {
        for (auto j = 0; j < Nb; j++) {
            out[i + 4 * j] = state[i * Nb + j];
        }
    }
}

template <AESKeyLength KeyLength>
void
AES<KeyLength>::decrypt_block(const uint8_t in[], uint8_t out[], const uint8_t round_keys[]) {
    uint8_t state[4 * Nb];

    // rotate the input block
    for (auto i = 0; i < 4; i++) {
        for (auto j = 0; j < Nb; j++) {
            state[i * Nb + j] = in[i + 4 * j];
        }
    }

    add_round_key(state, round_keys + Nr * 4 * Nb);

    for (auto round = Nr - 1; round >= 1; round--) {
        inv_sub_bytes(state);
        inv_shift_rows(state);
        add_round_key(state, round_keys + round * 4 * Nb);
        inv_mix_columns(state);
    }

    inv_sub_bytes(state);
    inv_shift_rows(state);
    add_round_key(state, round_keys);

    // rotate the block back
    for (auto i = 0; i < 4; i++) {
        for (auto j = 0; j < Nb; j++) {
            out[i + 4 * j] = state[i * Nb + j];
        }
    }
}

// Optimization of the cipher
// On systems with 32-bit or larger words, it is possible to speed up execution of this cipher by combining the SubBytes
// and ShiftRows steps with the MixColumns step by transforming them into a sequence of table lookups. This requires
// four 256-entry 32-bit tables (together occupying 4096 bytes). A round can then be performed with 16 table lookup
// operations and 12 32-bit exclusive-or operations, followed by four 32-bit exclusive-or operations in the AddRoundKey
// step. Alternatively, the table lookup operation can be performed with a single 256-entry 32-bit table (occupying
// 1024 bytes) followed by circular rotation operations. Using a byte-oriented approach, it is possible to combine the
// SubBytes, ShiftRows, and MixColumns steps into a single round operation.

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::shift_row(uint8_t row[], int n) {  // shift row on n positions
    uint8_t tmp[Nb];
    for (auto j = 0; j < Nb; j++) {
        tmp[j] = row[(j + n) % Nb];
    }
    memcpy(row, tmp, sizeof(tmp));
}

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::sub_bytes(uint8_t state[]) {
    for (auto i = 0; i < 4 * Nb; i++) {
        state[i] = SBOX[state[i] / 16][state[i] % 16];
    }
}

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::shift_rows(uint8_t state[]) {
    for (auto i = 1; i < 4; i++) {
        shift_row(state + i * Nb, i);
    }
}

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::mix_columns(uint8_t state[]) {
    uint8_t temp_state[4 * 4];
    memset(temp_state, 0, sizeof(temp_state));
    for (auto i = 0; i < 4; i++) {
        for (auto k = 0; k < Nb; k++) {
            for (auto j = 0; j < 4; j++) {
                temp_state[i * 4 + j] ^= GF_MUL_TABLE[CMDS[i][k]][state[k * Nb + j]];
            }
        }
    }
    memcpy(state, temp_state, sizeof(temp_state));
}

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::inv_sub_bytes(uint8_t state[]) {
    for (auto i = 0; i < 4 * Nb; i++) {
        state[i] = INV_SBOX[state[i] / 16][state[i] % 16];
    }
}

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::inv_mix_columns(uint8_t state[]) {
    uint8_t temp_state[4 * 4];
    memset(temp_state, 0, sizeof(temp_state));
    for (auto i = 0; i < 4; i++) {
        for (auto k = 0; k < Nb; k++) {
            for (auto j = 0; j < 4; j++) {
                temp_state[i * 4 + j] ^= GF_MUL_TABLE[INV_CMDS[i][k]][state[k * Nb + j]];
            }
        }
    }
    memcpy(state, temp_state, sizeof(temp_state));
}

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::inv_shift_rows(uint8_t state[]) {
    for (auto i = 1; i < 4; i++) {
        shift_row(state + i * Nb, Nb - i);
    }
}

// standard implementation
template <AESKeyLength KeyLength>
void
AES<KeyLength>::add_round_key(uint8_t state[], const uint8_t key[]) {
    for (auto i = 0; i < 4; i++) {
        for (auto j = 0; j < Nb; j++) {
            state[i * Nb + j] = state[i * Nb + j] ^ key[i + 4 * j];
        }
    }
}

template <AESKeyLength KeyLength>
void
AES<KeyLength>::key_expansion(const uint8_t key[], uint8_t w[]) {
    uint8_t temp[4];
    memcpy(w, key, 4 * Nk);

    // use lookup table without calculation
    // auto rcon = [](auto n) {
    //     uint8_t c = 1;
    //     for (auto i = 0; i < n - 1; i++) {
    //         c = (c << 1) ^ (((c >> 7) & 1) * 0x1b); // multiply on x
    //     }
    //     return c;
    // };

    auto rot_word = [](auto a) {
        uint8_t c = a[0];
        a[0] = a[1];
        a[1] = a[2];
        a[2] = a[3];
        a[3] = c;
    };

    auto sub_word = [](auto a) {
        a[0] = SBOX[a[0] / 16][a[0] % 16];
        a[1] = SBOX[a[1] / 16][a[1] % 16];
        a[2] = SBOX[a[2] / 16][a[2] % 16];
        a[3] = SBOX[a[3] / 16][a[3] % 16];
    };

    for (auto i = 4 * Nk; i < 4 * Nb * (Nr + 1); i += 4) {
        memcpy(temp, w + i - 4, 4);
        if (i / 4 % Nk == 0) {
            rot_word(temp);
            sub_word(temp);
            temp[0] ^= RCON[i / (Nk * 4)];
        } else if constexpr (Nk > 6)
            if (i / 4 % Nk == 4) {
                sub_word(temp);
            }

        for (auto j = 0; j < 4; j++) {
            w[i + j] = w[i + j - 4 * Nk] ^ temp[j];
        }
    }
}
